export default (functionContext) => {
    ...
    let outerCircle = helper.getPoints("circle", minDimension * 0.75, [w / 2, h / 2], data[0].length, data[0])
    let innerCircle = helper.getPoints("circle", minDimension * .6, [w / 2, h / 2], data[1].length, data[1])

    helper.drawPolygon(outerCircle.end, { close: true, radius: 4, lineColor: colors[2], color: colors[3] })
    helper.drawPolygon(innerCircle.end, { close: true, radius: 4, lineColor: colors[0], color: colors[1] })
